...
struct DrawHandler
{
	...
	void onSizeChanged (const CPoint& size)
	{
		...
		auto s = cairo_image_surface_create_for_data (static_cast<unsigned char*> (buffer),
													  CAIRO_FORMAT_ARGB32,
		 											  childWindow.getSize ().x, childWindow.getSize ().y,
													  childWindow.getBufferStride ());
...
struct Frame::Impl
{
	...
	: window (waylandFrame, size), drawHandler (window), frame (frame)//, dndHandler (&window, frame)
	{
...
//------------------------------------------------------------------------
Frame::Frame (IPlatformFrameCallback* frame, const CRect& size,
...
: IPlatformFrame (frame)
{
	...
	impl = std::unique_ptr<Impl> (new Impl (cfg ? cfg->waylandFrame : nullptr, {size.getWidth (), size.getHeight ()}, frame));
...
{
	return PlatformType::kWaylandSurface;
}
